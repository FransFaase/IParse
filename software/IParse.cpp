...
struct context_t
{	context_p parent;
	context_entry_p entries;
};

context_p cur_context = 0;

context_entry_p find_entry_in_context(context_p context, const char *ident_name, const char *ident_class)
{
	context_entry_p *r_entry;
	for (r_entry = &context->entries; 
	     *r_entry != 0; 
	     r_entry = &(*r_entry)->next)
		if (   (*r_entry)->ident_name == ident_name 
		    && (*r_entry)->ident_class == ident_class)
		    return *r_entry;

	(*r_entry) = new context_entry_t;
	(*r_entry)->next = 0;
	(*r_entry)->ident_name = ident_name;
	(*r_entry)->ident_class = ident_class;
	(*r_entry)->defs = 0;
	return *r_entry;
}

void build_contexts_for_tree(tree_t *tree, tree_t *parent_tree)
{	
	if (tree == 0)
		return;
	
	if (tree->type == tt_identdef || tree->type == tt_identdefadd)
	{
		const char *ident_name = tree->c.parts->first->c.str_value;
		const char *ident_class = tree->c.parts->next->first->c.str_value;
		context_entry_p entry;
		ident_def_p *r_def;
		tree_t *def_tree;
		
		if (cur_context == 0)
		{	printf("Error at %d.%d: no context for identifier '%s'\n",
				   tree->line, tree->column, ident_name);
			return;
		}
		
		entry = find_entry_in_context(cur_context, ident_name, ident_class);
		r_def = &entry->defs;
		
		if (tree->type == tt_identdef && (*r_def) != 0)
		{	//printf("Error at %d.%d: redefinition of identifier '%s' at %d.%d\n",
			//       tree->line, tree->column, ident_name,
			//       (*r_def)->tree->line, (*r_def)->tree->column);
			printf("Error at %d.%d: redefinition of identifier '%s' at ",
			       tree->line, tree->column, ident_name);
			if ((*r_def)->tree)
				printf("%d.%d\n", (*r_def)->tree->line, (*r_def)->tree->column);
			else
		...
		while ((*r_def) != 0) r_def = &(*r_def)->next;
		
		if (parent_tree == 0)
			def_tree = tree; // simply the identifier
		else
		{
			def_tree = parent_tree;
			if (nr_parts(parent_tree) == 2)
			{
				tree_t *first_child = part(parent_tree, 1), 
				       second_child = part(parent_tree, 2);
				if (first_child == tree && second_child != 0)
					def_tree = second_child;
				else if (second_child == tree && first_child != 0)
					def_tree = first_child;
			}	       
		}
		
		(*r_def) = new ident_def_t;
		(*r_def)->next = 0;
		(*r_def)->tree = def_tree;
	}
	else if (type_can_have_parts(tree->type))
	{	
		tree_t *parent_tree = tree->type == tt_list ? 0 : tree;
		list_t *list;
		
		for (list = tree->c.parts; list != 0; list = list->next)
		{	tree_t *elem_tree = list->first;
		
			if (elem_tree != 0 && elem_tree->type != 0)
			{	if (elem_tree->type == tt_opencontext)
				{	context_p new_context = new context_t;
				    new_context->parent = cur_context;
				    new_context->entries = 0;
				    elem_tree->c.context = new_context;
				    cur_context = new_context;
				}
				else if (elem_tree->type == tt_closecontext)
				{
					if (cur_context != 0)
						cur_context = cur_context->parent;
					else
						printf("Error at %d.%d: close context while no context is open\n",
							   elem_tree->line, elem_tree->column);
				}
				else
					build_contexts_for_tree(elem_tree, parent_tree);
			}
		}				
	}
}

void print_context(FILE *f, context_p context, bool compact)
{
	context_entry_p entry;
	if (context == 0 || context->entries == 0)
		return;
	fprintf(f, "<");
	for (entry = context->entries; entry != 0; entry = entry->next)
	{
		ident_def_p def;
		printf("%s:%s=", entry->ident_name, entry->ident_class);
		
		for (def = entry->defs; def != 0; def = def->next)
		{	if (def->tree)
				printf("%d.%d", def->tree->line, def->tree->column);
			if (def->next)
				fprintf(f, ",");
		}
		if (entry->next)
...
context_entry_p find_identifier(context_p context, 
								char *ident_name, char *ident_class)
{	context_entry_p entry;
	...
	for (entry = context->entries; entry != 0; entry = entry->next)
		if (   entry->ident_name == ident_name
		    && entry->ident_class == ident_class)
		    return entry;
	
	return find_identifier(context->parent, ident_name, ident_class);
}


void search_for_context_in_tree(tree_t *tree, context_p *rcontext, int *rcount)
{	int context_depth = 0;

	if (tree == 0)
		return;
		
	if (type_can_have_parts(tree->type))
	{	list_t *list = tree->c.parts;
		
		for (; list != 0; list = list->next)
		{	tree_t *child_tree = list->first;
	
			if (child_tree != 0 && child_tree->type != 0)
			{	if (child_tree->type == tt_opencontext)
				{	if (context_depth == 0)
					{	*rcontext = child_tree->c.context;
			    		(*rcount)++;
			    	}
			    	context_depth++; 
				}
				else if (child_tree->type == tt_closecontext)
					context_depth--;
				else if (context_depth == 0)
					search_for_context_in_tree(child_tree, rcontext, rcount);
			}
		}
	}
}

context_p find_local_context(ident_def_p defs)
{
	context_p result = 0;
	int count = 0;
	
	if (defs == 0 || defs->next != 0)
		return 0;
	
	search_for_context_in_tree(defs->tree, &result, &count);	
...
void verify_identifiers_in_list(list_t *list);

void verify_identifiers_in_tree(context_p *local_context, tree_t *tree)
{	
	if (tree == 0)
		return;
		
	if (tree->type == tt_identuse || tree->type == tt_identfield)
	{
		context_p used_context =   tree->type == tt_identuse
								 ? cur_context : *local_context;
		char *ident_name = tree->c.parts->first->c.str_value;
		char *ident_class = tree->c.parts->next->first->c.str_value;
		context_entry_p context_entry;

		if (verify_info)		
		{	printf("%d.%d check %s of %s in ", tree->line, tree->column,
				   ident_name, ident_class);
			print_context(stdout, used_context, true);
			printf("\n");
		}
		
		context_entry = find_identifier(used_context, ident_name, ident_class);
		if (context_entry == 0)
			printf("Error at %d.%d: no definition for identifier '%s'\n",
				   tree->line, tree->column, ident_name);
		else if (verify_info)
		{	context_p new_local_context;
			{	ident_def_p defs;
			
			    for (defs = context_entry->defs; defs != 0; defs = defs->next)
			    {
			    	if (defs->tree != 0 && verify_info)
					  	printf("Uses definition from %d.%d\n",
							   defs->tree->line, defs->tree->column);
				}
			}
		    new_local_context = find_local_context(context_entry->defs);
			if (new_local_context != 0)
				*local_context = new_local_context;
			else
				printf("Warning: no local context\n");
		}
	}
	else if (type_can_have_parts(tree->type))
	    verify_identifiers_in_list(tree->c.parts);
}

void verify_identifiers_in_list(list_t *list)
{	context_p local_context = 0;

	for (; list != 0; list = list->next)
	{	tree_t *tree = list->first;
	
		if (tree != 0 && tree->type != 0)
		{	if (tree->type == tt_opencontext)
			{	cur_context = tree->c.context;
				if (verify_info)
					printf("%d.%d: open context\n", tree->line, tree->column);
				local_context = 0;
			}
			else if (tree->type == tt_closecontext)
			{
				if (verify_info)
					printf("%d.%d: close context\n", tree->line, tree->column);
				if (cur_context != 0)
					cur_context = cur_context->parent;
				local_context = 0;	
			}
			else
				verify_identifiers_in_tree(&local_context, tree);
		}
	}				
}
#endif
