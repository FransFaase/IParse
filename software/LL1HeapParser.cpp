...
bool LL1HeapParser::parse_term( GrammarTerminal* term, AbstractParseTree &rtree )
{
...
bool LL1HeapParser::parse_ws_term( GrammarTerminal* term )
{
	TextFilePos start_pos = _text;
...
bool LL1HeapParser::parse_ident(GrammarIdent* ident, AbstractParseTree &rtree)
{	
...
protected:
	int _state;
	LL1HeapParser* _parser;
private:
	LL1HeapParseProcess* _parent_process;
};

class LL1HeapParseNTProcess : public LL1HeapParseProcess
{
public:
	LL1HeapParseNTProcess(LL1HeapParser* parser, GrammarNonTerminal* non_term, AbstractParseTree& rtree, bool &result)
		: LL1HeapParseProcess(parser), _non_term(non_term), _rtree(rtree), _result(result) {}
...
private:
	...
	Ident _nt;
	//ParseSolution* _sol;
	LL1HeapParseProcess* _sub_process;
	bool _sub_result;
    ParsedValue _val;
};

class LL1HeapParseOrRuleProcess : public LL1HeapParseProcess
{
public:
	LL1HeapParseOrRuleProcess(LL1HeapParser* parser, GrammarOrRule* or_rule, ParsedValue* prev_parts, AbstractParseTree &rtree, bool &result)
		: LL1HeapParseProcess(parser), _or_rule(or_rule), _prev_parts(prev_parts), _rtree(rtree), _result(result) {}
...
class LL1HeapParseRuleProcess : public LL1HeapParseProcess
{
public:
	LL1HeapParseRuleProcess(LL1HeapParser* parser, GrammarRule* rule, ParsedValue* prev_parts, const Ident tree_name, AbstractParseTree &rtree, bool &result)
		: LL1HeapParseProcess(parser), _rule(rule), _prev_parts(prev_parts), _tree_name(tree_name), _rtree(rtree), _result(result) {}
...
class LL1HeapParseSeqProcess : public LL1HeapParseProcess
{
public:
	LL1HeapParseSeqProcess(LL1HeapParser* parser, GrammarRule* rule, const char *chain_sym,
					AbstractParseTree seq, ParsedValue* prev_parts, const Ident tree_name,
					AbstractParseTree &rtree, bool &result)
		: LL1HeapParseProcess(parser), _rule(rule), _chain_sym(chain_sym), _seq(seq), _prev_parts(prev_parts), _tree_name(tree_name), _rtree(rtree), _result(result) {}
...
void LL1HeapParseNTProcess::execute()
{
	switch (_state) {
		case 1: goto state1;
		case 2: goto state2;
	...
	*/
	...
	{
		_sub_process = new LL1HeapParseRuleProcess(_parser, _or_rule->rule, (ParsedValue*)0, _or_rule->tree_name, _rtree, _sub_result);
	...
	if (_or_rule != 0)
	{
		...
		{   _val.prev = 0;
			...
			{
				_sub_process = new LL1HeapParseRuleProcess(_parser, _or_rule->rule, &_val, _or_rule->tree_name, _rtree, _sub_result);
				_parser->call(_sub_process);
...
void LL1HeapParseOrRuleProcess::execute()
{
	switch(_state) {
		case 1: goto state1;
	}
	...
	{
		_sub_process = new LL1HeapParseRuleProcess(_parser, _or_rule->rule, _prev_parts, _or_rule->tree_name, _rtree, _sub_result);
		_parser->call(_sub_process);
		_state = 1; return; state1:
		delete _sub_process;
		if (_sub_result)
...
void LL1HeapParseRuleProcess::execute()
{   
	switch(_state) {
		case 1: goto state1;
		case 2: goto state2;
	...
	*/
	...
	{   _start_pos = _parser->_text;
		...
		{   case RK_T_EOF:
        		...
        		if (_try_it)
        		{
					_sub_process = new LL1HeapParseRuleProcess(_parser, _rule->next, _prev_parts, _tree_name, _rtree, _sub_result);
					_parser->call(_sub_process);
					_state = 1; return; state1:
					delete _sub_process;
					if (_sub_result)
			...
			case RK_T_CLOSECONTEXT:
            	...
            	break;
			case RK_NT:
				_sub_process = new LL1HeapParseNTProcess(_parser, _rule->text.non_terminal, _t, _try_it);
				_parser->call(_sub_process);
				_state = 2; return; state2:
				delete _sub_process;
				break;
			...
			{   if (_rule->kind == RK_LIT)
				...
				{   if (_sequential)
					{   _seq.createList();
						...
						_sub_process = new LL1HeapParseSeqProcess(_parser, _rule, _chain_sym, _seq, _prev_parts, _tree_name, _rtree, _sub_result);
						_parser->call(_sub_process);
			...
			case RK_COLOURCODING:
				...
				break;
			case RK_OR_RULE:
				_sub_process = new LL1HeapParseOrRuleProcess(_parser, _rule->text.or_rules->first, (ParsedValue*)0, _t, _try_it);
				_parser->call(_sub_process);
				_state = 7; return; state7:
				delete _sub_process;
				break;
			case RK_COR_RULE:
				_sub_process = new LL1HeapParseOrRuleProcess(_parser, _rule->text.or_rules->first, _prev_parts, _rtree, _result);
				_parser->call(_sub_process);
		...
		{   /* We succeded in parsing the first element */
			...
			{   _seq.createList();
				...
				_sub_process = new LL1HeapParseSeqProcess(_parser, _rule, _chain_sym,
			...
			else
			{
				...
				_sub_process = new LL1HeapParseRuleProcess(_parser, _rule->next, &_val, _tree_name, _rtree, _sub_result);
				_parser->call(_sub_process);
...
void LL1HeapParseSeqProcess::execute()
{   
	switch(_state) {
		case 1: goto state1;
		case 2: goto state2;
...
/*void LL1HeapParser::init_solutions()
{	unsigned long i;
...
void LL1HeapParser::free_solutions()
{
	unsigned long i;
...
ParseSolution* LL1HeapParser::find_solution(unsigned long filepos, Ident nt)
{
	ParseSolution* sol;
...
void LL1HeapParser::expected_string(const char *s, bool is_keyword)
{
	AbstractParser::expected_string(_text, s, is_keyword);
}

void LL1HeapParser::call(LL1HeapParseProcess *parse_process)
{
	parse_process->_parent_process = _parse_process;
	_parse_process = parse_process;
}

void LL1HeapParser::exit()
{
	_parse_process = _parse_process->_parent_process;
}

bool LL1HeapParser::parse(const TextFileBuffer& textBuffer, Ident root_id, AbstractParseTree& rtree)
{
	_depth = 0;
	_text = textBuffer;
	_f_file_pos = _text;
	...
	bool try_it = false;
	LL1HeapParseProcess* _root = new LL1HeapParseNTProcess(this, findNonTerminal(root_id), rtree, try_it);
	call(_root);
	while (_parse_process != 0 && !_failed)
		_parse_process->execute();
	while (_parse_process != 0)
	{
		LL1HeapParseProcess* process = _parse_process;
